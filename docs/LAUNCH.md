...
```text
I built PlainLink because copied links are a useful place to remove tracking without requiring a browser extension. It watches the macOS clipboard, removes only known tracking parameters, and leaves unknown parameters alone so invite, checkout, login, and signed URLs are less likely to break. The last original URL can be restored from the menu bar.

The cleaning engine and CLI are written in Rust; the menu bar app is native Swift/AppKit. Rules are readable files with fixture-backed tests, and the app makes no network requests. Everything runs locally with no account or analytics.

This is an early developer preview for technical testers. The current app is not Developer ID-signed or notarized, so macOS will show a Gatekeeper warning; the README has the exact install steps and a source-build option. I would especially value examples of URLs it cleans incorrectly or should clean but does not.
```

The post and follow-up replies should explain:
...
1. GitHub Release.
2. GitHub repository Show HN submission.
